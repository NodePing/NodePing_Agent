    ...
    try {
        ...
        req.on('message', function (err, answer) {
            ...
            if (err) {
                ...
                debugMessage('info','check_dns: resolve response is: '+sys.inspect(answer));
                clearTimeout(timeoutid);
                if (!killit) {
                    killit = true;
                    ...
                    if (jobinfo.parameters.dnstoresolve && jobinfo.parameters.dnstoresolve != '') {
                        ...
                        var dnstypeid = answer.question[0].type;
                        if (jobinfo.parameters.contentstring && jobinfo.parameters.contentstring !== '' && jobinfo.parameters.contentstring != 'Optional') {
                            debugMessage('info','check_dns: Found Result Records: '+sys.inspect(answer.answer));
                            // Does it match?
                            for (var i=0;i<answer.answer.length;i++) {
                                if (answer.answer[i].type == dnstypeid || dnstype == 'ANY') {
                                    var found = false;
                                    if (answer.answer[i].name == jobinfo.parameters.contentstring) {
                                        debugMessage('info','check_dns: '+jobinfo.parameters.contentstring+' equals "name" '+answer.answer[i].name);
                                        found = true;
                                    } else if (answer.answer[i].address == jobinfo.parameters.contentstring) {
                                        debugMessage('info','check_dns: '+jobinfo.parameters.contentstring+' equals "address" '+answer.answer[i].address);
                                        found = true;
                                    } else if (answer.answer[i].primary == jobinfo.parameters.contentstring) {
                                        debugMessage('info','check_dns: '+jobinfo.parameters.contentstring+' equals "primary" '+answer.answer[i].primary);
                                        found = true;
                                    } else if (answer.answer[i].data == jobinfo.parameters.contentstring) {
                                        debugMessage('info','check_dns: '+jobinfo.parameters.contentstring+' equals "data" '+answer.answer[i].data);
                                        found = true;
                                    } else if (answer.answer[i].exchange == jobinfo.parameters.contentstring) {
                                        debugMessage('info','check_dns: '+jobinfo.parameters.contentstring+' equals "exchange" '+answer.answer[i].exchange);
                                        found = true;
                                    } else if (answer.answer[i].target == jobinfo.parameters.contentstring) {
                                        debugMessage('info','check_dns: '+jobinfo.parameters.contentstring+' equals "target" '+answer.answer[i].target);
                                        found = true;
                                    }
                                    if (found) {
                                        req = null;
                                ...
                                debugMessage('info','check_dns: '+jobinfo.parameters.contentstring+' does not match '+sys.inspect(answer.answer[i.toString()]));
                            }
                            // No match, fail
                            req = null;
                            jobinfo.results.success = false;
                            ...
                            jobinfo.results.diag.dns.answer = prepAnswerForDiags(answer.answer);
                            ...
                            for (var i=0;i<answer.answer.length;i++) {
                                if (answer.answer[i].type == dnstypeid || dnstype == 'ANY') {
                                    if (answer.answer[i].name == jobinfo.parameters.dnstoresolve) {
                                        found = true;
                                        break;
                                    }
                                }
                        ...
                        debugMessage('info','check_dns: Server responded: '+sys.inspect(answer.answer));
                        req = null;
                        jobinfo.results.success = true;
                        jobinfo.results.statusCode = 'Success';
